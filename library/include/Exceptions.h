...
namespace MedicalClinic
{
	...
	{
	public:
		explicit NullPointerException(const std::string& pointerName) : std::logic_error(
			"Wskaźnik " + pointerName + " ma wartość nullptr.")
		{
		};
	};
	...
	{
	public:
		explicit NoExistException(const std::string& name, const std::string& uniqueParametr) : std::logic_error(
			"Nie istnieje " + name + " o takim parametrze: " + uniqueParametr)
		{
		};
	};
	...
	{
	public:
		explicit ExistException(const std::string& name, const std::string& uniqueParametr) : std::logic_error(
			"Istnieje " + name + " o takim parametrze: " + uniqueParametr)
		{
		};
	};
	...
	{
	public:
		explicit OpeningException(const std::string& fileName) : std::logic_error(
	...
	{
	public:
		explicit EmptyRecordException(const std::string& name) : std::logic_error(
			"Próba zapisu pustego wektora: " + name)
		{
		};
	};
	...
	{
	public:
		explicit WriteException(const std::string& fileName) : std::logic_error(
			"Błąd strumienia podczas zapisywania do pliku: " + fileName)
		{
		};
	};
	...
	{
	public:
		explicit UnexpectedCharacterException(const std::string& clas) : std::logic_error(
			clas + " Wczytano nieoczekiwany znak.")
		{
		};
	};
	...
	{
	public:
		explicit ArchiveArchivedException(const std::string& clas) : logic_error(
			"Nie można zaarchiwiować zarchiwizowanego " + clas)
		{
		};
	};
}
