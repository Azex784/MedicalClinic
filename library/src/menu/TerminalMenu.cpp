...
namespace RehabClinic
{
	...
	bool TerminalMenu::dismissPersonnel() const
	{
		...
		cout << "Podaj identyfikator specialisty, który ma zostać zaarchiwiowany: ";
		getline(cin, tmp2);
		if (!checkInt(tmp2, personnelId)) return false;
		...
		try
		{
			...
			cout << "Sukces! Udało się zaarchiwizować specialistę: " << getLogicManager()->getPersonnelManager()->
	...
	void TerminalMenu::personnel() const
	{
		...
		while (!exit)
		{
			...
			cout << "<3> - zaarchiwizuj specjalistę" << endl;
